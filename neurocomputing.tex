...
The hyper parameters of the various methods are set using a k-fold on a dataset set that is then never used in the predictive performance estimate. We try the values for diffusion parameter in DK and MED in $\lbrace 10^{-3}, 10^{-3}, 10^{-2}, 10^{-1} \rbrace$, time steps in MD in $\lbrace 1, 10, 100 \rbrace$ and RL parameter in $\lbrace 1, 4, 7 \rbrace$. For CDNK, we try for the degree threshold values in $\lbrace 10,\ 15,\ 20 \rbrace$, clique size threshold in $\lbrace 4,\ 5 \rbrace$, maximum radius in $\lbrace 1,\ 2 \rbrace$, maximum distance in $\lbrace 2,\ 3,\ 4 \rbrace$, nuber of clusters $K$ in $\lbrace 5,\ 7 \rbrace$. Finally, the regularization trade off parameter $C$ for the SVM is searched in $\lbrace 10^{-5},  \ 10^{-4}, \ 10^{-3},\ 10^{-2},\ 10^{-1}, 1,\ 10,\ 10^2 \rbrace$.
\section{Results and discussion}
Table \ref{table_biogps} and \ref{table_pathways} show the AUC performance of the models trained by using different graph node kernels. According to the performance, we can group them into two groups: diffusion-based kernels and CDNK variations. CDNK variations are ranked at first in all cases on BioGPS dataset and 9 out of 11 cases on Pathways. CDNK is the top performant kernel when considering both the average AUC-ROC and the average rank with a difference w.r.t. the best performance of diffusion-based kernels ranging from 5.4$\%$ to 10$\%$ and from 1.2$\%$ to 3.2$\%$ on BioGPS and Pathways, respectively. Regarding variations of CDNK, the integration of real valued vector information improves the performance of CDNK in most cases comparing to the case of using discrete labels only. The improvement in average ranges from 1.3$\%$ to 4.6$\%$. It is worth to notice that the average ranks of CDNK with the use of real vector information are high in both ways of discrete labeling and on both databases.
